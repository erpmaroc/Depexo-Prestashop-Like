...
AeReg
- New script!
- Separate regroup/resize operations and add to module `depexoTools.cluster`
...
- Update documentation and associated [website](http://depexotools.rtfd.io/)
...
=======
General
- Begin preparation for a major update (Depexo 3.0)
...
AeRes
- refactored functionality into `depexoTools/AeRes.py`
...
- publish to [PyPi](https://pypi.org/project/depexoTools/)
...
* depexo
  ...
  * if a region file is supplied but cannot be used Depexo will now complain
...
* AeRes
  * NEW program. The aim of AeRes is to produce residual images for Depexo.
  * AeRes will take an input catalog, and an input image, and produce a residual image, and an optional model image.
    * Catalog can be any format that Depexo writes.
...
* depexo
  ...
  * `--priorized` fitting has been updated
    * `--catpsf=CATPSF` allows the user to supply an image which describes the psf of the input catalog. This will allow Depexo to deconvolve the sources in the input catalog, and (re) convolve the sources with the image psf to get the correct source sizes.
    ...
    * Priorized fitting uses the same fitting techniques as depexos blind source finding. This includes the use of an inverse covariance matrix.
...
* MIMAS
  * New option `--mim2fits` will now output a correctly formatted Milti-Order Coverage map (MOC) according to [[IVOA recommendation|http://ivoa.net/documents/MOC/]]. These footprint files can be visualised by Aladin. Reading of these fits files has not **yet** been implemented in any of the Depexo tools.
* BANE
...
* depexoTools
  * A lot of code has been moved out of ``Depexo.py`` and into modules within the ``depexoTools`` directory. If you are used to importing Depexo to have access to some functionality, you should now be able to import the same functionality from a smaller module within ``depexoTools``.
...
* depexo
  ...
  * use `--telescope <name>` to let Depexo lookup the latitude from a list of known telescopes
    * Depexo currently knows about MWA|ATCA|VLA|LOFAR

### v1.9rc1-104-gbfe836c [Feb 4 2015]

* SR6 - NEW!
 * Shrink Ray-6 is a command line wrapper for the new `depexoTools.fits_interp` module
 ...
 * `fits_interp` handles the compression/expansion of fits files using decimation and interpolation
   * when combined with BANE and Depexo `fits_interp` allows for a great reduction in the size of the background and noise images.
 ...
 * added new command line option `--autoload`
   * `--autoload` will look for background (_bkg.fits), noise (_rms.fits), and region (.mim) files that have the same base name as the input image. If found these files will be loaded, if not, Depexo proceeds as if you had not specified them.
...
* depexo
 * `find_sources_in_image` now accepts mask=region as either a filename or an instance of `depexoTools.regions.Region`

### v1.9rc1-66-g2fdde1a [Jan 13 2015]

* depexo
 * VOTables, databases, and .reg files now contain metadata
  * Currently this is just the Depexo version
