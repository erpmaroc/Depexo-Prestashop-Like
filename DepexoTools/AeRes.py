#! /usr/bin/env python
"""
Depexo Residual (AeRes) has the following capability:
...
from depexoTools import catalogs, fitting, wcs_helpers
...
def load_sources(filename,
    ...
    Return
    ------
    catalog : [`class:depexoTools.models.ComponentSource`, ...]
...
def make_model(sources, shape, wcshelper, mask=False, frac=None, sigma=4):
    ...
    Parameters
    ----------
    sources : [`class:depexoTools.models.ComponentSource`, ...]
        a list of sources
...
def make_residual(fitsfile, catalog, rfile, mfile=None, add=False, mask=False, frac=None, sigma=4,
    ...
    catalog : str
        Input catalog filename of a type supported by depexo
