#! /usr/bin/env python
"""
Provide fitting routines and helper fucntions to depexo
"""
...
def jacobian(pars, x, y):
    ...
    See Also
    --------
    :func:`depexoTools.fitting.emp_jacobian`
    """
...
def emp_jacobian(pars, x, y):
    ...
    See Also
    --------
    :func:`depexoTools.fitting.jacobian`
    """
    eps = 1e-5
    matrix = []
...
def lmfit_jacobian(pars, x, y, errs=None, B=None, emp=False):
    """
    Wrapper around :func:`depexoTools.fitting.jacobian` and :func:`depexoTools.fitting.emp_jacobian`
    ...
    B : 2d-array
        a B-matrix (optional) see :func:`depexoTools.fitting.Bmatrix`
    ...
    See Also
    --------
    :func:`depexoTools.fitting.Bmatrix`
    :func:`depexoTools.fitting.jacobian`
    :func:`depexoTools.fitting.emp_jacobian`
...
def hessian(pars, x, y):
    ...
    See Also
    --------
    :func:`depexoTools.fitting.emp_hessian`
    """
...
def emp_hessian(pars, x, y):
    ...
    Notes
    -----
    Uses :func:`depexoTools.fitting.emp_jacobian` to calculate the first order derivatives.

    See Also
    --------
    :func:`depexoTools.fitting.hessian`
    """
    eps = 1e-5
    matrix = []
    for i in range(pars['components'].value):
...
def bias_correct(params, data, acf=None):
    ...
    See Also
    --------
    :func:`depexoTools.fitting.RB_bias`
    """
...
def do_lmfit(data, params, B=None, errs=None, dojac=True):
    ...
    See Also
    --------
    :func:`depexoTools.fitting.lmfit_jacobian`
...
if __name__ == "__main__":
    ...
    def plot_jacobian():
        ...
        params.add('c0_xo', value=1. * nx / 2, min=nx / 2. -
                   smoothing / 2., max=nx / 2. + smoothing / 2)
        params.add('c0_yo', value=1. * ny / 2, min=ny / 2. -
                   smoothing / 2., max=ny / 2. + smoothing / 2.)
    ...
    def test_hessian_plots():
        ...
        kwargs = {"interpolation": "nearest",
                  'aspect': 1, 'vmin': -1, 'vmax': 1}
        fig, ax = pyplot.subplots(
            6, 6, squeeze=True, sharex=True, sharey=True, figsize=(5, 6))
    ...
    def test_jacobian_plot():
        ...
        fig, ax = pyplot.subplots(
            6, 3, sharex=True, sharey=True, figsize=(3, 6))
