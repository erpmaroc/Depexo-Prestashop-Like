...
class WCSHelper(object):
    ...
    def from_header(cls, header, beam=None, psf_file=None):
        ...
        beam : :class:`depexoTools.wcs_helpers.Beam` or None
        ...
        Returns
        -------
        obj : :class:`depexoTools.wcs_helpers.WCSHelper`
            A helper object.
        """
        try:
            wcs = WCS(header, naxis=2)
...
def get_beam(header):
    """
    Create a :class:`depexoTools.wcs_helpers.Beam` object from a fits header.
...
def fix_aips_header(header):
    ...
    header["BMIN"] = bmin
    header["BPA"] = bpa
    header["HISTORY"] = "Beam information AIPS->fits by depexoTools"
    return header
